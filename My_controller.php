...
class MY_controller extends Controller
{
    ...
    {
		$a = DB::select( DB::raw("SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '". \Config::get('database.connections.'.\Config::get('database.default').'.database') ."' AND TABLE_NAME = '". \DB::getTablePrefix() ."l_vendorassociatedcategories_commissions' AND COLUMN_NAME ='commission' ") );
		if ($a[0]->DATA_TYPE  == "int" )
		{
			DB::statement('ALTER TABLE `'. \DB::getTablePrefix() .'l_vendorassociatedcategories_commissions` CHANGE `commission` `commission` DOUBLE UNSIGNED NULL DEFAULT NULL; ');
		}
		
		
		$a = DB::select( DB::raw("SHOW COLUMNS FROM `". \DB::getTablePrefix() ."l_site_settings` LIKE 'sellercenter_notes'") );
		if (count($a) <= 0 )
		{			
			DB::statement('ALTER TABLE `'. \DB::getTablePrefix() .'l_site_settings` ADD `sellercenter_notes` TEXT NOT NULL AFTER `sellercenter_meta_title`; ');
		}
        ...
        $this->mage_defaultStoreId          = Mage::app()->getWebsite(true)->getDefaultGroup()->getDefaultStoreId();
        $this->mage_TreeRootId              = Mage_Catalog_Model_Category::TREE_ROOT_ID;
        $this->mage_CategoryRootId          = Mage::app() -> getDefaultStoreView() -> getRootCategoryId();
                
        
        
        $this->mage_defaultWebsiteId           = Mage::app()->getStore($this->mage_defaultStoreId)->getWebsiteId();
    ...
    public function default_data()
    {
		...
		$data['_global_VENDOR_ID']								= FALSE;
		$data['_global_USER_ID']								= FALSE;
		if( RoleManagement::is_Vendor() ) 
		{
			$data['_global_VENDOR_ID']							= RoleManagement::getCurrent_LoggedInID();
		}
		else
		{
			$data['_global_USER_ID']							= RoleManagement::getCurrent_LoggedInID();
		}
    ...
    public function _auth_current_logged_in_ID( $compare_with )
    {
        if ( $compare_with == RoleManagement::getCurrent_LoggedInID() )
        {
            return TRUE;
        }
